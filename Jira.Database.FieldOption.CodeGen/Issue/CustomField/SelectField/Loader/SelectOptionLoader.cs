...
namespace lazyzu.Jira.Database.FieldOption.CodeGen.Issue.CustomField.Select.Loader
{
    internal interface ISelectOptionLoader
    {
        Task<ImmutableArray<SelectOption>> LoadFieldOptions(decimal fieldId, CancellationToken cancellationToken);
    }

    internal static class SelectOptionLoader
    {
        public static ISelectOptionLoader LoadFrom(DatabaseConfig databaseConfig)
        {
            ...
            if ("mysql".Equals(databaseConfig.Type, StringComparison.OrdinalIgnoreCase)) return new MySqlSelectOptionLoader(databaseConfig.ConnectionString);
