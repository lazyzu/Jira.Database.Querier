<template>
    ...
    <div v-if="loading !== undefined">
        <p v-if="loading">Loading from In Component Query...</p>
        <div v-else>
            <div v-if="error">
                <p> Query is failed </p>
                <p> {{ error.name }}</p>
...
import { type InComponentQuery_IssueQueryQuery as IssueQueryResponseType } from "../gql/codegen/graphql"
...
const issueQuery = graphql(`query InComponentQuery_IssueQuery($key: String)
{
    issue(key: $key)
    {
        id,
