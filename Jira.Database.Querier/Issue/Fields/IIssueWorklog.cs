using Generator.Equals;
using lazyzu.Jira.Database.EntityFrameworkCore;
using lazyzu.Jira.Database.Querier.Issue.Contract;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading;
using System.Threading.Tasks;

namespace lazyzu.Jira.Database.Querier.Issue.Fields
{
    public interface IIssueWorklog
    {
        public decimal Id { get; }
        public string Author { get; }
        public DateTime? Created { get; }

        public string UpdateAuthor { get; }
        public DateTime? Updated { get; }

        public DateTime? Started { get; }
        public TimeSpan? TimeSpent { get; }

        public string Comment { get; }
    }

    [Equatable(Explicit = true)]
    public partial class IssueWorklog : IIssueWorklog
    {
        [DefaultEquality]
        public decimal Id { get; init; }

        public string Author { get; init; }

        public DateTime? Created { get; init; }

        public string UpdateAuthor { get; init; }

        public DateTime? Updated { get; init; }

        public DateTime? Started { get; init; }

        public TimeSpan? TimeSpent { get; init; }

        public string Comment { get; init; }

        public override string ToString()
        {
            return Comment;
        }
    }

    public class IssueWorklogProjection : IIssueExternalProjectionSpecification
    {
        public IEnumerable<FieldKey> HandleTarget { get; protected init; }

        protected readonly JiraContext jiraContext;
        protected readonly ILogger logger;

        public IssueWorklogProjection(JiraContext jiraContext, ILogger logger)
        {
            this.jiraContext = jiraContext;
            this.logger = logger;

            HandleTarget = new FieldKey[]
            {
                IssueFieldSelection.Worklogs
            };
        }

        public virtual async Task Projection(IEnumerable<JiraIssue> issues, CancellationToken cancellationToken = default)
        {
            var _issues = issues?.ToArray() ?? new JiraIssue[0];

            if (_issues.Any())
            {
                var issueIds = _issues.Select<JiraIssue, decimal?>(issue => issue.Id).ToArray();

                var issueWorklogMap = await LoadIssueWorklogMap(issueIds, cancellationToken).ConfigureAwait(false);

                if (issueWorklogMap.Any())
                {
                    foreach (var issue in _issues)
                    {
                        if(issueWorklogMap.TryGetValue(issue.Id, out var worklogs)) issue.Worklogs = worklogs;
                        else issue.Worklogs = new IIssueWorklog[0];
                    }
                }
                else foreach (var issue in _issues)
                {
                    issue.Worklogs = new IIssueWorklog[0];
                }
            }
        }

        protected virtual async Task<Dictionary<decimal?, IssueWorklog[]>> LoadIssueWorklogMap(decimal?[] issueIds, CancellationToken cancellationToken)
        {
            var query = jiraContext.worklog.AsNoTracking()
                .Where(worklog => issueIds.Contains(worklog.issueid))
                .Select(dbModel => new
                {
                    dbModel.ID,
                    dbModel.issueid,
                    dbModel.AUTHOR,
                    dbModel.CREATED,
                    dbModel.UPDATEAUTHOR,
                    dbModel.UPDATED,
                    dbModel.STARTDATE,
                    dbModel.timeworked,
                    dbModel.worklogbody
                });

            var queryResult = await query.ToArrayAsync(cancellationToken).ConfigureAwait(false);

            return queryResult.GroupBy(dbModel => dbModel.issueid)
                .ToDictionary(issueIdGroup => issueIdGroup.Key
                            , issueIdGroup => issueIdGroup.Select(dbModel =>
                            {
                                return new IssueWorklog
                                {
                                    Id = dbModel.ID,
                                    Author = dbModel.AUTHOR,
                                    Created = dbModel.CREATED,
                                    UpdateAuthor = dbModel.UPDATEAUTHOR,
                                    Updated = dbModel.UPDATED,
                                    Started = dbModel.STARTDATE,
                                    TimeSpent = dbModel.timeworked.HasValue ? TimeSpan.FromSeconds((double)dbModel.timeworked.Value) : null,
                                    Comment = dbModel.worklogbody,
                                };
                            }).ToArray());
        }
    }
}
