...
{
    internal class IssuePriorityOptionGenerator : INativeFieldSelectionGenerator
    {
        ...
        {
            var options = LoadIssuePriorityOption_FromMySql(databaseConfig.ConnectionString, sourceProductionContext.CancellationToken).Result;  // TODO: support MYSQL Only currently
            ...
            if (options != null && options.Any())
            {
                ...
                codeBuilder.AppendLine($@"    public static class IssuePrioritySelection");
                codeBuilder.AppendLine(@"    {");
                foreach (var option in options)
                {
                    var optionName = variableNameBuilder.BuildVariableName(option.Name, option.Id);

                    codeBuilder.AppendLine($@"        public static IIssuePriority {optionName} = new IssuePriority");
                    codeBuilder.AppendLine(@"        {");
                ...
                sourceProductionContext.AddSource($"SimpleFieldSelection.IssuePrioritySelection.g.cs", sourceText);
            }
        }

        private async Task<ImmutableArray<IssuePriority>> LoadIssuePriorityOption_FromMySql(string connectionString, CancellationToken cancellationToken)
        {
            using (var dbConnection = new MySqlConnector.MySqlConnection(connectionString))
            {
                ...
                return queryResult.Select(option => new IssuePriority()
                {
                    Id = option.ID,
                    Name = option.pname,
